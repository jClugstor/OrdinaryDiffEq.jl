@muladd function _ode_addsteps!(
        k, t, uprev, u, dt, f, p, cache::OwrenZen4Cache,
...
@muladd function _ode_addsteps!(
        k, t, uprev, u, dt, f, p, cache::OwrenZen5Cache,
        always_calc_begin = false, allow_calc_end = true,
        force_calc_end = false
    )
...
@muladd function _ode_addsteps!(
        k, t, uprev, u, dt, f, p, cache::DP5Cache,
        always_calc_begin = false, allow_calc_end = true,
        force_calc_end = false
    )
...
"""
@muladd function _ode_addsteps!(
        k, t, uprev, u, dt, f, p, cache::BS5Cache,
        always_calc_begin = false, allow_calc_end = true,
        force_calc_end = false
    )
    if length(k) < 8 || always_calc_begin
    ...
    if (allow_calc_end && length(k) < 11) || force_calc_end # Have not added the extra stages yet
        ...
        @.. broadcast = false tmp = uprev +
            dt * (
            a91 * k[1] + a92 * k[2] + a93 * k[3] + a94 * k[4] +
                a95 * k[5] + a96 * k[6] + a97 * k[7] + a98 * k[8]
        )
        f(rtmp, tmp, p, t + c6 * dt)
        copyat_or_push!(k, 9, rtmp)
        @.. broadcast = false tmp = uprev +
            dt *
            (
...
@muladd function _ode_addsteps!(
        k, t, uprev, u, dt, f, p, cache::OwrenZen3Cache,
        always_calc_begin = false, allow_calc_end = true,
        force_calc_end = false
    )
    if length(k) < 4 || always_calc_begin
...
@muladd function _ode_addsteps!(
        k, t, uprev, u, dt, f, p, cache::DP5ConstantCache,
        always_calc_begin = false, allow_calc_end = true,
        force_calc_end = false
    )
    if length(k) < 4 || always_calc_begin
...
"""
@muladd function _ode_addsteps!(
        k, t, uprev, u, dt, f, p, cache::BS5ConstantCache,
        always_calc_begin = false, allow_calc_end = true,
        force_calc_end = false
    )
    if length(k) < 8 || always_calc_begin
