...
function perform_step!(integrator, cache::RKMK4Cache, repeat_step = false)
    ...
    if alg.krylov
        ...
        u .= exponential!(
            (1 / 6) * (k1 + 2 * k2 + 2 * k3 + k4 - (k1 * k4 - k4 * k1) / 2),
            exp_method,
            exp_cache
        ) * uprev
    end
...
function perform_step!(integrator, cache::CG3Cache, repeat_step = false)
    ...
    u .= (
        exponential!(dt * (24 / 17) * C, exp_method, exp_cache) *
...
function perform_step!(integrator, cache::CG2Cache, repeat_step = false)
    ...
    k2 = dt * convert(AbstractMatrix, L)
    u .= (
        exponential!((1 / 2) * (k1), exp_method, exp_cache) *
            (exponential!((1 / 2) * k2, exp_method, exp_cache))
    ) * uprev
...
function perform_step!(integrator, cache::CG4aCache, repeat_step = false)
    ...
    u .= (
        exponential!(dt * (0.3322195591068374) * E, exp_method, exp_cache) *
            exponential!(dt * (-0.1907142565505889) * D, exp_method, exp_cache) *
            exponential!(dt * (0.739781398537078) * C, exp_method, exp_cache) *
            exponential!(dt * (-0.018369853156402) * B, exp_method, exp_cache) *
            exponential!(dt * (0.1370831520630755) * A, exp_method, exp_cache)
    ) * uprev
...
function perform_step!(integrator, cache::MagnusNC8Cache, repeat_step = false)
    ...
    Q7 = (-1 / 42) * (
        B0 * (B0 * (Q3 - Q4 / 3 + dt * Q5) - (Q3 - Q4 / 3 + dt * Q5) * B0) -
...
function perform_step!(
        integrator, cache::MagnusLeapfrogCache, repeat_step = false,
...
function perform_step!(
        integrator, cache::LinearExponentialConstantCache,
        repeat_step = false
    )
